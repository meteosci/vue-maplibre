...
export default function (instance?) {
  ...
  return {
    log: makeLog(`[VueMaplibre] ${instance?.proxy?.$options.name}`),
    warn: makeWarn(`[VueMaplibre] WARN ${instance?.proxy?.$options.name}`),
    error: makeError(`[VueMaplibre] ERR ${instance?.proxy?.$options.name}`),
    debug: makeDebug(`[VueMaplibre] Debug ${instance?.proxy?.$options.name}`),
    capsule,
    success,
    warning,
    danger,
